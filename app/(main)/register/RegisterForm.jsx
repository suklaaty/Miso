...
export default function LoginForm() {
    ...
    async function handleRegister(e) {
        ...
        e.preventDefault()
        let errors = []
        let request = await UseApi('register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email, password: password }) })
    ...
    return (
        <Container maxWidth='xs' sx={{ mt: 10 }} disableGutters>
            <Typography variant='h4' sx={{ mb: 2, pb: 1, fontWeight: 'bold', textAlign: 'center' }}>Register | Miso</Typography>
            <Box component='form' onSubmit={handleRegister}>
                ...
                <Button type='submit' fullWidth variant='contained' sx={{ mt: 2 }} disabled={isLoading}>{isLoading ? <CircularProgress size={24} /> : 'Register'}</Button>
